...
def create_test_auto_matrix():
    ...
    for i, group in enumerate(groups):
        ...
        try:
            ...
            if len(questions_in_group) < count:
                errors.append(f"Nhóm {i+1} (Filters: {filters}): Yêu cầu {count} câu, chỉ tìm thấy {len(questions_in_group)}.")
    ...
    # 6. Tính điểm (dùng lại hàm cũ)
    points_map = calculate_question_points(all_question_ids, db)

    # 7. Định dạng mảng câu hỏi và đếm type (dùng lại logic cũ)
    formatted_questions = []
