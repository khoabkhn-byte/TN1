...
def create_test_auto_matrix():
    ...
    for i, group in enumerate(groups):
        ...
        if filters.get("tags"):
            # Dùng logic $in giống hệt hàm /api/questions của bạn
            match_query["tags"] = {"$in": [filters["tags"].strip()]}

        # 4. Thêm logic loại bỏ các câu hỏi đã được chọn
        if all_question_ids_found:
            # Lấy ID (ưu tiên 'id' (UUID), fallback về str(_id))
            # Chúng ta cần loại bỏ cả hai
            ...
            # Thêm điều kiện $nin (not in) vào $match
            # Dùng $and để đảm bảo tất cả các điều kiện được áp dụng
            ...
            # Chỉ gán $and nếu có điều kiện
            if len(and_conditions) > 0:
                 # Gộp $and với query gốc (nếu query gốc đã có $and)
        ...
        try:
            ...
            # 🔥 FIX: Cảnh báo Tiếng Việt
            if len(questions_in_group) < count:
                filters_str_parts = []
                if filters.get("type"):
                    filters_str_parts.append(f"Loại: {filters['type']}")
                if filters.get("difficulty"):
                    filters_str_parts.append(f"Độ khó: {filters['difficulty']}")
                if filters.get("tags"):
                    filters_str_parts.append(f"Tag: {filters['tags']}")
    ...
    new_test = {
        ...
        "level": level,
        "questions": formatted_questions,
        "isAutoGenerated": True, # Vẫn là tự động
        "generationConfig": data.get("groups"), # Lưu lại cấu hình ma trận
        "createdAt": now_vn_iso(), # Dùng hàm now_vn_iso() đã có
        "mcCount": mc_count,
        "essayCount": essay_count,
        "tfCount": tf_count,
        "fillCount": fill_count,
    ...
    try:
        ...
        # Trả về kèm theo các lỗi (nếu có)
...
def preview_auto_test_matrix():
    ...
    if not all_questions_found:
         # 🔥 FIX: Trả về success=False và warnings (để FE đọc được)
