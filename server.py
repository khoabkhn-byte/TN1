...
def create_test():
    ...
    # 1. Lấy các câu hỏi từ DB dựa trên _id
    object_ids = []
    ...
    # Tạo map {str(_id): id (UUID)}
    id_map = {str(q.get("_id")): q.get("id") for q in questions_from_db if q.get("id")}
    
    # Tạo danh sách ID (UUID) cuối cùng để lưu
    # Giữ nguyên thứ tự ban đầu từ FE
    ...
    for oid_str in question_oids_from_fe:
        ...
        if uuid:
            ...
            # Fallback nếu câu hỏi không có 'id' (UUID) (trường hợp hiếm)
    ...
    for q_id in question_uuids_to_save: # Giữ nguyên thứ tự
    ...
    new_test = {
        ...
        "level": level,
        "subject": subject,
        "questions": formatted_questions, # ✅ Mảng câu hỏi đã chứa id (UUID) và điểm
